#Main code that takes from input both the anti-nu spectrum and the background (only geo-nu)
...
Bg=fn.ReadFlux(InputBkgFile, N)*time
...
for i in range(0,M):
    ...
    if(Fluc==False):
        yNO=fn.FluxNO_ResEn_teo_bg(e, F, Bg, Theta13_NO, Theta12, DeltaM21, DeltaM31_NO, D, Ntot, a, b, c)

    #with statistical fluctuations
    else:
        yNO=fn.FluxNO_ResEn(e, F, Bg, Theta13_NO, Theta12, DeltaM21, DeltaM31_NO, D, Ntot, a, b, c)
    ...
    def least_squares_sist_NO(T13, T12, M21, M3l, dist, N, A, B, C,alpha_C,alpha_D,alpha_R,alpha_B):
        ym = fn.FluxNO_ResEn_teo(e, F, T13, T12, M21, M3l, dist, N, A, B, C)    #without background
        z=(yNO-ym*(1+alpha_C+alpha_D+alpha_R)-Bg*(1+alpha_B))**2/(yNO+(sigma_b2b*ym)**2+(sigma_sh*Bg)**2)
        CHI=np.sum(z)
        pull_a=((A-a)/sigma_a)**2
        pull_b=((B-b)/sigma_b)**2
        pull_c=((C-c)/sigma_c)**2
        return CHI+pull_a+pull_b+pull_c+(alpha_C/sigma_alphaC)**2+(alpha_D/sigma_alphaD)**2+(alpha_R/sigma_alphaR)**2+(alpha_B/sigma_B)**2
    least_squares_sist_NO.errordef = Minuit.LEAST_SQUARES
    m = Minuit(least_squares_sist_NO, T13=Theta13_NO,T12=Theta12,M21=DeltaM21,M3l=DeltaM31_NO,dist=D,N=Ntot,A=a,B=b,C=c,alpha_C=0,alpha_D=0,alpha_R=0, alpha_B=0)
    ...
    #Fit IO su NO
    def least_squares_sist_IO(T13, T12, M21, M3l, dist, N, A, B, C,alpha_C,alpha_D,alpha_R,alpha_B):
        ym = fn.FluxIO_ResEn_teo(e, F, T13, T12, M21, M3l, dist, N, A, B, C)    #without background
        z=(yNO-ym*(1+alpha_C+alpha_D+alpha_R)-Bg*(1+alpha_B))**2/(yNO+(sigma_b2b*ym)**2+(sigma_sh*Bg)**2)
        CHI=np.sum(z)
        pull_a=((A-a)/sigma_a)**2
        pull_b=((B-b)/sigma_b)**2
    ...
    n = Minuit(least_squares_sist_IO, T13=Theta13_IO,T12=Theta12,M21=DeltaM21,M3l=DeltaM32_IO,dist=D,N=Ntot,A=a,B=b,C=c,alpha_C=0,alpha_D=0,alpha_R=0,alpha_B=0)
...
if(Fluc==False):
    ...
    if(corr==True):
        ...
        plt.savefig("MASFit_scan_1.pdf")

    #Print on file of fit reconstructed parameters
    file_par = open("MASFit_parameters_1.txt", "w")
    ...
    if(sist==True):
        valuesNO=np.delete(m.values,[9,10,11,12])
        valuesIO=np.delete(n.values,[9,10,11,12])
    else:
        valuesNO=m.values
        valuesIO=n.values
    plt.plot(xe, fn.FluxNO_ResEn_teo_bg(e,F,Bg,*valuesNO), label="fit NO", color="seagreen")
    plt.plot(xe, fn.FluxIO_ResEn_teo_bg(e,F,Bg,*valuesIO), label="fit IO", color="coral")
